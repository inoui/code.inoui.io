(function() {
  ...
  if ( !window.__st['offsite'] ) {
    if (readCookie('_inoui_visit')) { visit = 1; }
    if (readCookie('_inoui_uniq')) { uniq = 1; }

    // set return visit cookie
    var expire_time_in_msec = today.getTime() + (30*60*1000); // expire 30 minutes from now 
    writeCookie('_inoui_visit', 't', expire_time_in_msec);
    ...
    writeCookie('_inoui_uniq', 'x', expire_time);

    uniqToken = fetch(true, '_inoui_y', function() { return uniqueId(); });
    visitToken = fetch(false, '_inoui_s', function() { return uniqueId(); });
  }
