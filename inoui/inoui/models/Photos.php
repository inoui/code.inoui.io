...
class Photos extends \lithium\core\StaticObject {
    
    public static $infoCache = array();


    public static function generate($slug, $type, $crop, $w = null, $h = null) {


        $path = LITHIUM_APP_PATH . "/webroot/media/{$slug}";
        $source = $path . "." . $type;
        $width = (int) $w;
        $height = (int) $h;
        ...
        $thumbs = LITHIUM_APP_PATH . "/webroot/media/thumbs";
        // $destination = "{$path}.{$width}{$crop}{$height}.{$type}.thumb";
        $destination = "{$thumbs}/{$file}.{$width}{$crop}{$height}.{$type}";
        ...
        if ($crop == 'c') {
            self::image_scale_cropped($source, $destination, $width, $height);
        } else {
            self::image_scale($source, $destination, $width, $height);
        }
        return file_get_contents($destination);
...
//            return false;
        ...
        if ($end_height > $height || $end_width > $width) self::image_gd_resize_white($source, $destination, $width, $height, $end_width, $end_height);
        else self::image_gd_resize($source, $destination, $width, $height);
        // self::image_gd_resize($source, $destination, $width, $height);
    }
    ...
    public static function image_scale_cropped($source, $destination, $width, $height) {
      ...
      /* We are not square. */
        ...
        self::image_gd_resize($source, $destination, $width, $height,  $source_x, $source_y, $source_width, $source_height);
    }
    ...
    protected static function image_gd_resize($source, $destination, $width, $height, $source_x = 0, $source_y = 0, $source_width = null, $source_height = null) {
      ...
      protected static function image_gd_resize_white($source, $destination, $width, $height, $endWidth = 0, $endHeight = 0) {
        if (!file_exists($source)) {
            return false;
        }
        if (!$info = self::image_get_info($source)) {
        ...
        $w = max($width, $endWidth);
        $h = max($height, $endHeight);
        ...
        // echo $y;
        // die();
        imagecopyresampled($res, $im, $x, $y, 0, 0, $width, $height,  $source_width, $source_height);
        if (!imageistruecolor($im)) {
            imagetruecolortopalette($res, false, 256);
        }
        $result = self::image_gd_write($res, $destination, $info['format']);
