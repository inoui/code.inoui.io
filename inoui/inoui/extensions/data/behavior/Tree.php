...
class Tree extends \li3_behaviors\data\model\Behavior {
	...
	public function createTree($entity, $childrens, $nLeft = 0, $nRight = null) {
		extract($this->_config);
	    $tree = array();
	    foreach ($childrens as $child) {
	        if ($child[$left] == $nLeft + 1 && (is_null($nRight) || $child[$right] < $nRight)) {
	            $tree[$child['_id']] = $child;
				$tree[$child['_id']]['children'] =  $entity->createTree($childrens, $child[$left], $child[$right]);
	            $nLeft = $child[$right];
	        }
	    }
	    return $tree;
	...
	public function nestedChildrens($entity, $rec = null, $mode = 'all') {
	
		extract($this->_config);
		$childrens = $entity->childrens(true, 'all');
		$aChildrens = $childrens->to('array');
		$tree = $entity->createTree($aChildrens, $entity->$left, $entity->$right);
	    return $tree;
 ...
 */
	...
	protected function _beforeSave($params) {
		...
		if (!$entity->data($model::key())) {
			...
			if ($entity->$parent == $params['data'][$parent]) {
				return true;
			}
			...
			$this->_updateNode($entity, $params['data'][$parent]);
		}
		return true;
	}
	 ...
	 */
	protected function _updateNode($entity, $parentId) {
		extract($this->_config);
		...
		if ($entity->$parent !== null) {
			$newParent = $this->_getById($parentId);
			if ($newParent) {
	...
	protected function _update($rght, $dir = '+', $span = 2, $scp = array()) {
		...
		$model::update(array('$inc'=>array($right => intval($dir . $span))), array(
			$right => array('>=' => $rght)
		) + $scp);

		$model::update(array('$inc'=>array($left => intval($dir . $span))), array(
			$left => array('>' => $rght)
		) + $scp);
